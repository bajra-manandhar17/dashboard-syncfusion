        ...
        <>
          <div className="flex items-center justify-between">
            ...
            <TooltipComponent content='Menu' position='BottomCenter'>
              <button type='button' onClick={() => setActiveMenu((prevActiveMenu) => !prevActiveMenu)} className='block p-3 mt-4 text-xl rounded-full hover:bg-light-gray md:hidden'>
                <MdOutlineCancel />
              </button>
            </TooltipComponent>
          </div>
          <div className='mt-10'>
            {links.map((item) => (
              <div key={item.title}>
                ...
                {item.links.map((link) => (
                  <NavLink key={link.title} to={`/${link.name}`} onclick={handleCloseSideBar} className={({ isActive }) => isActive ? activeLink : normalLink}>
                    {link.icon}
